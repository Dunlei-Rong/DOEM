...
def response_parsing(response, dataset='Mashup'):
    ...
    api_list = []
    if dataset == 'Mashup':
        with open('rubbish/Mashup/preprocessed_data/api_names', 'rb') as f:
...
class LambdaRankLoss(nn.Module):
    """
    Metric-aware pairwise ranking loss.

    Core idea:
    For a ranked list, if swapping positions i and j improves the metric-oriented
    utility, then the model should reduce s_i - s_j; otherwise it should enlarge it.

    This keeps the "metric-specific swap impact" idea from the paper, but avoids
    treating LambdaRank pseudo-gradients as a scalar loss.

    Assumptions:
    - api_tag_embedding: [num_api, num_tags], binary multi-hot preferred.
      If it is not binary, it will be thresholded to {0,1} by > 0.
    - y_scores: [batch, topn] or list of [topn]
    - y_preds:  [batch, topn] or list of [topn], aligned with y_scores
    - M:        [batch, num_tags] or list of [num_tags], binary multi-hot demand vector
    """

    def __init__(
        self,
        api_tag_embedding: torch.Tensor,
        mode: str = "cover",
        topn: Optional[int] = None,
        metric_weights: Optional[Dict[str, float]] = None,
        normalize_each_metric: bool = True,
        reinforce_correct_pairs: bool = True,
        eps: float = 1e-8,
    ):
        super().__init__()

        api_tag_embedding = (api_tag_embedding > 0).float()
        self.register_buffer("api_tag_embedding", api_tag_embedding)

        self.num_api, self.tag_num = api_tag_embedding.shape
        self.mode_list = [m for m in mode.split("+")]
        self.topn = topn
        self.normalize_each_metric = normalize_each_metric
        self.reinforce_correct_pairs = reinforce_correct_pairs
        self.eps = eps

        default_weights = {
            "cover": 1.0,
            "efficiency": 1.0,   # precision-like utility
            "redundancy": 1.0,   # 1 - redundancy
            "tolerance": 1.0,    # repeated support on demand tags
            "count": 0.0,        # optional auxiliary term
        }
        if metric_weights is not None:
            default_weights.update(metric_weights)
        self.metric_weights = default_weights

    def forward(self, y_scores, y_preds, M):
        # y_scores: List[Tensor] or Tensor
        # y_preds:  List[Tensor] or Tensor
        # M:        Tensor or List[Tensor]

        # ---- keep variable-length lists as lists; do NOT stack them ----
        if isinstance(y_scores, torch.Tensor):
            score_list = [row for row in y_scores]
        else:
            score_list = list(y_scores)

        if isinstance(y_preds, torch.Tensor):
            pred_list = [row for row in y_preds]
        else:
            pred_list = list(y_preds)

        if isinstance(M, torch.Tensor):
            demand_list = [row for row in M]
        else:
            demand_list = list(M)

        # find device from the first score tensor
        device = score_list[0].device
        api_tag_embedding = self.api_tag_embedding.to(device)

        total_loss = torch.tensor(0.0, device=device)
        valid_count = 0

        for scores, preds, demand in zip(score_list, pred_list, demand_list):
            scores = scores.to(device)
            preds = preds.to(device).long()
            demand = demand.to(device).float()
            ...
            if self.topn is not None:
                scores = scores[:self.topn]
                preds = preds[:self.topn]

            # 防御：空样本直接跳过
            if preds.numel() == 0 or scores.numel() == 0:
                continue
            ...
            sample_loss = self._compute_sample_loss(
                ...
                preds=preds,
                demand=demand,
                api_tag_embedding=api_tag_embedding,
            )

            if torch.isnan(sample_loss).any() or torch.isinf(sample_loss).any():
                continue
    ...
    def _compute_sample_loss(
        ...
        api_tag_embedding: torch.Tensor,   # [num_api, d]
    ) -> torch.Tensor:
        tags = api_tag_embedding[preds.long()]   # [n, d], binary
        n = tags.shape[0]

        if n <= 1:
            return scores.new_tensor(0.0)

        base_utility = self._list_utility(tags, demand)

        pair_losses: List[torch.Tensor] = []

        for i in range(n):
            for j in range(i + 1, n):
                swapped_tags = tags.clone()
                swapped_scores = scores.clone()

                swapped_tags[[i, j]] = swapped_tags[[j, i]]
                swapped_scores[[i, j]] = swapped_scores[[j, i]]

                swapped_utility = self._list_utility(swapped_tags, demand)
                delta = swapped_utility - base_utility

                # If delta > 0, swapping improves the metric => current order is wrong
                # and we want score_j > score_i.
                # If delta < 0, current order is better and we may reinforce it.
                score_diff = scores[i] - scores[j]

                if delta > 0:
                    # Want j ahead of i => penalize large (s_i - s_j)
                    pair_loss = delta * F.softplus(score_diff)
                    pair_losses.append(pair_loss)
                elif delta < 0 and self.reinforce_correct_pairs:
                    # Want i ahead of j => penalize large (s_j - s_i)
                    pair_loss = (-delta) * F.softplus(-score_diff)
                    pair_losses.append(pair_loss)
                else:
                    continue
    ...
    def _list_utility(self, tags: torch.Tensor, demand: torch.Tensor) -> torch.Tensor:
        """
        tags:   [n, d], binary multi-hot for ranked APIs
        demand: [d], binary multi-hot for required tags

        Utility is computed on all prefixes with logarithmic discount.
        Higher is better.
        """
        device = tags.device
        n, d = tags.shape
        demand = (demand > 0).float()

        demand_cnt = torch.count_nonzero(demand).float().clamp_min(1.0)

        # prefix union for cover / efficiency / dependence
        prefix_union = torch.cummax(tags, dim=0).values  # [n, d]

        # per-prefix tag counts
        union_cnt = torch.count_nonzero(prefix_union, dim=1).float().clamp_min(1.0)
        per_item_cnt = torch.count_nonzero(tags, dim=1).float()
        prefix_item_sum = torch.cumsum(per_item_cnt, dim=0).clamp_min(1.0)

        # log discount: 1 / log2(k+2), k from 0
        discount = 1.0 / torch.log2(torch.arange(2, n + 2, device=device).float())

        total_utility = tags.new_tensor(0.0)
        active_weight = 0.0

        # ----- cover -----
        if "cover" in self.mode_list:
            # fraction of demand tags already covered
            covered = torch.count_nonzero(prefix_union * demand.unsqueeze(0), dim=1).float()
            cover_u = covered / demand_cnt
            cover_u = (cover_u * discount).sum()
            total_utility = total_utility + self.metric_weights["cover"] * cover_u
            active_weight += self.metric_weights["cover"]

        # ----- efficiency / save -----
        # Utility version of "how much of the selected content is useful"
        if "efficiency" in self.mode_list or "save" in self.mode_list:
            useful = torch.count_nonzero(prefix_union * demand.unsqueeze(0), dim=1).float()
            efficiency_u = useful / union_cnt
            efficiency_u = (efficiency_u * discount).sum()
            total_utility = total_utility + self.metric_weights["efficiency"] * efficiency_u
            active_weight += self.metric_weights["efficiency"]

        # ----- dependence -----
        # Redundancy ratio = (sum individual tags - unique tags) / sum individual tags
        # Since lower redundancy is better, utility = 1 - redundancy = unique / sum
        if "redundancy" in self.mode_list:
            dependence_u = union_cnt / prefix_item_sum
            dependence_u = (dependence_u * discount).sum()
            total_utility = total_utility + self.metric_weights["redundancy"] * dependence_u
            active_weight += self.metric_weights["redundancy"]

        # ----- tolerance -----
        # Demand tags supported by at least two selected APIs.
        # This better matches "fault tolerance" than pairwise-any union/intersection tricks.
        if "tolerance" in self.mode_list:
            prefix_support = torch.cumsum(tags, dim=0)                  # [n, d]
            tolerant_mask = ((prefix_support >= 2).float() * demand.unsqueeze(0))
            tolerant_cnt = torch.count_nonzero(tolerant_mask, dim=1).float()
            tolerance_u = tolerant_cnt / demand_cnt
            tolerance_u = (tolerance_u * discount).sum()
            total_utility = total_utility + self.metric_weights["tolerance"] * tolerance_u
            active_weight += self.metric_weights["tolerance"]

        # ----- count -----
        # Optional: reward earlier full coverage of demand.
        if "count" in self.mode_list and self.metric_weights["count"] > 0:
            covered = torch.count_nonzero(prefix_union * demand.unsqueeze(0), dim=1).float()
            full_mask = covered >= demand_cnt
            if full_mask.any():
                first_idx = torch.nonzero(full_mask, as_tuple=False)[0, 0].float()
                count_u = 1.0 / torch.log2(first_idx + 2.0)
            else:
                count_u = tags.new_tensor(0.0)
            total_utility = total_utility + self.metric_weights["count"] * count_u
            active_weight += self.metric_weights["count"]

        if self.normalize_each_metric and active_weight > 0:
            total_utility = total_utility / (active_weight + self.eps)
